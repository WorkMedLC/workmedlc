      ...
      <nav class="row flex items-center md-max:flex-wrap">
        <div class="left lg:pr-4 md-max:w-full md-max:flex md-max:items-center md-max:justify-between">
          <a href="/" class="flex items-center">
            <div>
              <g-image src="~/assets/images/logo-icon.png" alt="WorkMed Inc." class="md-max:p-2" width="63" /> 
            </div>
        ...
        <div class="right  md-max:w-full" :style="{height: mobileHeight}">
          ...
          <ul class="nav-links md:flex uppercase font-header">
            ...
            :class="{'has-children': link.subNav}">
              ...
              <ul class="subnav md:absolute z-10 md:bg-white md:w-48 md:opacity-0 md:pointer-events-none"
              v-if="link.subNav">
                <li class="relative"
                v-for="(sublink, i) in link.subNav"
                :key="`sublink-${i}`"
                >
                  <g-link class="block border-l-2 border-white 
...
export default {
  data:() => ({ 
    ...
    navLinkClasses: 'nav-link py-6 px-4 border-white block md:border-t-2 hover:text-orange-300  md:hover:border-orange-400 md-max:py-4 md-max:px-0 ',
    navOpen: false
  }),
  methods: {
    openNav(evt){
